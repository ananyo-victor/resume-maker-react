  ...
  return (
    <div>
      <div className='absolute z-30 right-0 h-screen w-fit flex justify-center items-center px-3 opacity-50 hover:opacity-100 hover:from-white bg-gradient-to-l from-blue-300 to-white'>
        <button className='size-fit p-3 text-2xl font-semibold drop-shadow-2xl shadow-2xl rounded-xl bg-white hover:bg-blue-700 hover:text-white' disabled={location.pathname === '/review'} type='submit' onClick={handleNextClick}><Link to={des}>Next</Link></button>
      </div>
    </div>
  )
}
